...
static void info_win__draw(window* this) {
	window_draw(this);
	this->at(this, 3, 0 );
	this->puts(this, "Информация");
	this->at(this, 3, 2 );
	this->puts(this, "Это окно");
	this->at(this, 5, 3 );
	this->puts(this, "не");
	this->at(this, 1, 4 );
	this->puts(this, "актививно!");
}

void multiMenuExec(struct menuv* m) {
	//
	menuv_init(&m1,m1items,&m1rect); m1.header = "[ Меню 1 ]";
	menuv_init(&m2,m2items,&m2rect); m2.header = "[ Меню 2 ]";
	menuv_init(&m3,m3items,&m3rect); m3.header = "[ Меню 3 ]";
	//
